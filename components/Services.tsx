...
const SERVICES = [
  {
    n: '01', icon: Zap, status: 'live', flag: 'MAGIC',
    title: 'Magic — выплата курьеру',
    sub: 'Главный endpoint. Один вызов = все проверки + фискализация.',
    detail: 'Передайте ИИН курьера, сумму и order_id. За миллисекунды: валидация, проверка статуса, контроль лимита 300 МРП, загрузка чека в Webkassa. Возврат: PROCEED / WARNING / BLOCK с детальной причиной.',
    code: 'POST /process-payment',
  },
  {
    n: '02', icon: Search, status: 'live',
    title: 'Проверка СНР и ОКЭД',
    sub: 'Реестр налогоплательщиков КГД и stat.gov.kz в одном вызове.',
    detail: 'Введите БИН или ИИН — получите наименование, ОКЭД, тип регистрации, дату начала деятельности, налоговый орган. Источник — официальные API КГД с fallback на структуру БИН.',
    code: 'GET /taxpayer/:bin',
  },
  {
    n: '03', icon: Users, status: 'live',
    title: 'Статус ИП / ФЛ / ТОО',
    sub: 'Тип юридической формы из реестра.',
  ...
  },
  {
    ...
    detail: 'Математическая проверка по Постановлению Правительства РК № 853 от 26.08.2013. Возвращает: валидность, дату рождения, пол, век, серию. Без сети, мгновенно.',
    code: 'POST /iin/validate',
  },
  {
    n: '05', icon: XCircle, status: 'live',
    title: 'Аннулирование заказа',
    sub: 'Soft cancel до фискализации курьером.',
    ...
    code: 'POST /cancel-order',
  },
  {
    n: '06', icon: Receipt, status: 'live',
    title: 'Статус фискального чека',
    sub: 'QR-код и фискальный номер от КГД.',
    detail: 'После фискализации курьером Webkassa отправляет нам webhook с фискальным номером и временем. Вы получаете готовую ссылку на QR-код от КГД для приложения к договору.',
    code: 'GET /receipts/:order_id',
  },
  {
    n: '07', icon: TrendingUp, status: 'live',
    title: 'Лимит 300 МРП в месяц',
    sub: 'Контроль превышения по самозанятому в реальном времени.',
    detail: 'Накопительный счётчик доходов каждого курьера. При приближении к лимиту — WARNING. При попытке превысить — BLOCK с предложением «курьер должен зарегистрироваться как ИП».',
    code: 'GET /income-limit/*',
  },
  {
    n: '08', icon: Database, status: 'demo',
    title: 'Реестр самозанятых',
    sub: 'Прямой доступ к реестру КГД ИСНА.',
    detail: 'Поиск зарегистрированных самозанятых по ИИН и регионам. Проверка статуса до приёма курьера на платформу. Подключение через договор с КГД (в работе).',
    code: 'GET /self-employed/registry',
  },
  {
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          ...
          <h2 className="text-4xl md:text-6xl font-bold text-navy tracking-tight leading-[1.1] text-balance">
            Каждое требование НК РК 2026 — <span className="text-navy/40">отдельный endpoint.</span>
          </h2>
          <p className="mt-6 text-lg text-navy/60 leading-relaxed">
            Можете использовать сервисы по отдельности или вызывать
            магический <code className="px-2 py-0.5 mx-1 rounded bg-navy-50 text-navy font-mono text-sm">/process-payment</code>,
            который объединяет 7 проверок в один запрос с миллисекундным ответом.
          </p>
        </motion.div>
